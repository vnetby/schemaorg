<?php

/**
 * @see https://schema.org/BodyMeasurementBust
 * Maximum girth of bust. Used, for example, to fit women's suits.
 */

namespace Vnetby\Schemaorg\Types\Thing\Intangible\Enumeration\MeasurementTypeEnumeration\BodyMeasurementTypeEnumeration;

class BodyMeasurementBust extends \Vnetby\Schemaorg\Types\Thing\Intangible\Enumeration\MeasurementTypeEnumeration\BodyMeasurementTypeEnumeration\BodyMeasurementTypeEnumeration
{
    const TYPE = 'BodyMeasurementBust';
}
