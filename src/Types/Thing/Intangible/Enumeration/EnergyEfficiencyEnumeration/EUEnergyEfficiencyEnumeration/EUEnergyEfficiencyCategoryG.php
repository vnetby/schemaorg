<?php

/**
 * @see https://schema.org/EUEnergyEfficiencyCategoryG
 * Represents EU Energy Efficiency Class G as defined in EU energy labeling regulations.
 */

namespace Vnetby\Schemaorg\Types\Thing\Intangible\Enumeration\EnergyEfficiencyEnumeration\EUEnergyEfficiencyEnumeration;

class EUEnergyEfficiencyCategoryG extends \Vnetby\Schemaorg\Types\Thing\Intangible\Enumeration\EnergyEfficiencyEnumeration\EUEnergyEfficiencyEnumeration\EUEnergyEfficiencyEnumeration
{
    const TYPE = 'EUEnergyEfficiencyCategoryG';
}
